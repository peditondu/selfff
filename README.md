...
echo \
  ...
  [https://download.docker.com/linux/$](https://download.docker.com/linux/$)(. /etc/os-release; echo "$ID") \
...
Installa Tailscale Ovunque: Vai sul sito di Tailscale e installa l'app sul tuo server e su tutti i dispositivi da cui vorrai usare Stremio (telefono, PC, tablet, etc.).

Accedi con lo Stesso Account: È fondamentale usare lo stesso account di login su tutti i dispositivi.

Abilita la MagicDNS ✨: Vai alla pagina DNS della tua console Tailscale e assicurati che queste due opzioni siano abilitate:

MagicDNS

HTTPS Certificates

Trova il Nome del Tuo Server: Nella pagina Machines, trova il tuo server e copia il suo nome macchina completo (es. tuoserver.tailxxxx.ts.net).
